...
function groupSignalsBySegment(
  ...
  for (const signalList of Array.from(map.values())) {
    signalList.sort((a: Signal, b: Signal) => a.offsetT - b.offsetT);
  }
...
function findBlocks(
  ...
  for (const sub of subSegments) {
    ...
    const block: Block = {
      ...
      segmentIds: Array.from(new Set(blockSegments.map(s => s.parentSegmentId))),
...
/**
 * Assign unique colors to blocks using greedy graph coloring
 */
function assignBlockColors(blocks: Block[]): void {
  const colors = [
    '#ff6b6b', '#4ecdc4', '#45b7d1', '#f9ca24', '#f0932b',
    '#eb4d4b', '#6ab04c', '#c44569', '#f8b500', '#38ada9',
    '#b33771', '#3c6382', '#60a3bc', '#e55039', '#4a69bd',
  ];
...
function countJunctionsInBlock(
  ...
  let junctionCount = 0;
  for (const [nodeId, degree] of Array.from(nodeDegrees.entries())) {
    if (degree >= 3) {
      junctionCount++;
    }
  }
