import { Signal, Segment, Node, Issue, Point } from '../types/railway';
...
export function computeSignalPosition(
  ...
  if (segment.geometry === 'straight') {
    ...
    // Bezier curve (handled later when implementing curves)
...
export function findSignalOverlaps(
  signals: Signal[],
  minDistance: number // centimeters
): Issue[] {
  const issues: Issue[] = [];
  ...
  for (const [segmentId, segmentSignals] of Array.from(signalsBySegment.entries())) {
    for (let i = 0; i < segmentSignals.length; i++) {
      for (let j = i + 1; j < segmentSignals.length; j++) {
        ...
        // Convert to world units (approximate for straight segments)
        // For precise measurement, compute actual segment length
        if (distance < 0.01) { // ~1% of segment = overlap
          issues.push({
...
export function findSignalConflicts(
  ...
  for (const [segmentId, segmentSignals] of Array.from(signalsBySegment.entries())) {
    const hasAtoB = segmentSignals.some((s: Signal) => s.orientation === 'AtoB');
    const hasBtoA = segmentSignals.some((s: Signal) => s.orientation === 'BtoA');
    ...
    if (hasAtoB && hasBtoA) {
      issues.push({
        ...
        elementIds: segmentSignals.map((s: Signal) => s.id),
...
/**
 * Find segment under mouse cursor
 */
export function findSegmentAtPoint(
  worldPos: Point,
  segments: Segment[],
  nodeMap: Map<string, Node>,
  threshold: number = 400 // centimeters
): Segment | null {
  for (const segment of segments) {
    ...
    if (segment.geometry === 'straight') {
      const dist = pointToLineDistance(worldPos, nodeA, nodeB);
      if (dist < threshold) {
        return segment;
      }
    }
    // TODO: Handle bezier curves when implemented
  }
...
/**
 * Helper: point-to-line distance for straight segments
 */
...
/**
 * Helper for curved segments (stub for now)
 */
